...
Vec4f vec4FromAIColor4D(aiColor4D &v) { return Vec4f(v.r, v.g, v.b, v.a); }

Vec3f vec3FromAIVector3D(aiVector3D &v) { return Vec3f(v.x, v.y, v.z); }

Vec2f vec2FromAIVector3D(aiVector3D &v) { return Vec2f(v.x, v.y); }
...
void initLogStream() {
  ...
  if (!initializedLog) {
    ...
    logStream = aiGetPredefinedLogStream(aiDefaultLogStream_STDOUT, nullptr);
...
public:
  Impl(const aiScene *scene)
      : /* SceneNode::Impl(scene->mRootNode),*/ scene(scene) {
    nodes.resize(countNodes(scene->mRootNode));
    addNode(scene->mRootNode, 0);
  }

  ~Impl() { aiReleaseImport(scene); }
...
Scene::Material ::Material()
    : two_sided(0), wireframe(0), diffuse(0.6), ambient(0), specular(1.),
      emissive(0.), shininess(0.25), shading_model(0), blend_func(0),
      shininess_strength(1), opacity(1), reflectivity(0), refracti(0),
      bump_scaling(1), transparent(0.), reflective(0.) {}
...
void Scene::verbose(bool b) { aiEnableVerboseLogging(b); }
...
Scene ::Scene(Impl *impl) : mImpl(impl) {
  ...
  for (unsigned int i = 0; i < materials(); i++) {
    ...
    max = 4;
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_DIFFUSE, m.diffuse.components,
                            &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_AMBIENT, m.ambient.components,
                            &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_SPECULAR,
                            m.specular.components, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_EMISSIVE,
                            m.emissive.components, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_TRANSPARENT,
                            m.transparent.components, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_REFLECTIVE,
                            m.reflective.components, &max);
    aiGetMaterialFloat(mat, AI_MATKEY_OPACITY, &m.opacity);
    aiGetMaterialFloat(mat, AI_MATKEY_SHININESS, &m.shininess);
    aiGetMaterialFloat(mat, AI_MATKEY_SHININESS_STRENGTH,
                       &m.shininess_strength);
...
Scene ::~Scene() { delete mImpl; }

unsigned int Scene ::meshes() const { return mImpl->scene->mNumMeshes; }
...
void Scene ::mesh(unsigned int i, Mesh &mesh) const {
  if (i < meshes()) {
    ...
    if (amesh) {
      ...
      bool hasnormals = amesh->mNormals != nullptr;
      bool hascolors = amesh->mColors[0] != nullptr;
      bool hastexcoords = amesh->mTextureCoords[0] != nullptr;
...
unsigned int Scene ::materials() const { return mImpl->scene->mNumMaterials; }

unsigned int Scene ::textures() const { return mImpl->scene->mNumTextures; }

unsigned int Scene ::nodes() const { return mImpl->nodes.size(); }

Scene::Node &Scene ::node(unsigned int i) const { return mImpl->nodes[i]; }

void get_bounding_box_for_node(const aiScene *scene, const struct aiNode *nd,
                               Vec3f &min, Vec3f &max, aiMatrix4x4 *trafo) {
  aiMatrix4x4 prev;
  unsigned int n = 0, t;
  prev = *trafo;
...
void Scene ::getBounds(Vec3f &min, Vec3f &max) const {
  ...
  get_bounding_box_for_node(mImpl->scene, mImpl->scene->mRootNode, min, max,
                            &trafo);
}

void dumpNode(aiNode *x, std::string indent) {
  printf("%sNode (%s) with %d meshes (", indent.c_str(), x->mName.data,
         x->mNumMeshes);
...
void Scene ::print() const {
  ...
  for (unsigned int i = 0; i < mImpl->scene->mNumMeshes; i++) {
    ...
    printf("\t\t%d vertices, %d faces; material: %d; normals?%d colors?%d "
           "texcoords?%d\n",
           x->mNumVertices, x->mNumFaces, x->mMaterialIndex, x->HasNormals(),
           x->HasVertexColors(0), x->HasTextureCoords(0));
    // printf("\t\tcolors: %d, normals %d, texcoords %d\n", x->mColors[0][0] !=
    // NULL, x->mNormals[0] != NULL, x->mTextureCoords[0][0] != NULL);
  }
  ...
  for (unsigned int i = 0; i < mImpl->scene->mNumMaterials; i++) {
    ...
    for (unsigned int j = 0; j < x->mNumProperties; j++) {
      ...
      int dim;
      std::string str;
      printf("\t\t%d: %s = { texture: %d, semantic: %d } ", j, p->mKey.data,
             p->mIndex, p->mSemantic);
      switch (p->mType) {
      case aiPTI_Float:
